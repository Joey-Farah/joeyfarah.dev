  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-3 py-2 bg-white/5 border-b border-brand-primary/10">
        ...
        <span className="ml-2 text-xs text-brand-text/70">
      ...
      <div className="px-4 py-3 flex items-center gap-6 flex-wrap">
        {loading && (
          <span className="text-xs text-brand-text/60 animate-pulse">fetching...</span>
        )}
        {!loading && !stats && (
          <span className="text-xs text-brand-text/60">unavailable</span>
        )}
        {stats && (
          <>
            <div className="flex flex-col">
              <span className="text-xs text-brand-text/70">// rating</span>
              <span className="text-lg font-bold text-brand-primary leading-tight">
                {stats.rating.toLocaleString()}
              </span>
              <span className="text-xs text-brand-text/75 leading-tight">{stats.rankName}</span>
            </div>
            {stats.globalRank && (
              <div className="flex flex-col">
                <span className="text-xs text-brand-text/70">global rank</span>
            ...
            <div className="flex flex-col">
              <span className="text-xs text-brand-text/70">W / L</span>
            ...
            <div className="flex flex-col">
              <span className="text-xs text-brand-text/70">win rate</span>
              <span className="text-sm font-bold text-brand-text/85 leading-tight">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t border-brand-primary/15 bg-brand-primary/[0.04] text-xs">
        <span className="text-brand-text/75">slippi.gg/user/joey-870</span>
