...
describe('ingestSeed — production blocks.seed.json', () => {
  ...
  it('hero lines contain the Melee Diamond rank line (Story 32)', async () => {
    const blocks = await ingestSeed(BLOCKS_SEED);
    const hero = blocks.find((b) => b.slug === 'hero');
    const content = hero!.content as { lines: string[] };
    expect(content.lines.some((l) => l.includes('Diamond'))).toBe(true);
  });

  it('hero lines contain the 4+ years tenure line (Story 32)', async () => {
    const blocks = await ingestSeed(BLOCKS_SEED);
    const hero = blocks.find((b) => b.slug === 'hero');
    const content = hero!.content as { lines: string[] };
    expect(content.lines.some((l) => l.includes('4+'))).toBe(true);
  });
});
